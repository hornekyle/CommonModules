module autoDiffZArray_mod
	use autoDiffZType_mod
	use autoDiffZOperator_mod
	implicit none
	private
...
contains
	...
	function maxloc_a1(u,idx) result(o)
		type(adZ_t),dimension(:),intent(in)::u
	...
	pure function sum_a1(u) result(o)
		...
		o = adZ_t( (0.0_wp,0.0_wp) , size(u(1)%grad()) )
	...
	pure function sum_a2(u) result(o)
		...
		o = adZ_t( (0.0_wp,0.0_wp) , size(u(1,1)%grad()) )
	...
	pure function sum_a3(u) result(o)
		...
		o = adZ_t( (0.0_wp,0.0_wp) , size(u(1,1,1)%grad()) )
	...
	pure function matmul_a21(u,v) result(o)
		type(adZ_t),dimension(:,:),intent(in)::u
		type(adZ_t),dimension(:),intent(in)::v
		type(adZ_t),dimension(:),allocatable::o
	...
	function TDMA_sA(A,b) result(x)
		...
		type(adZ_t),dimension(:,:),intent(in)::A
			!! Coefficient matrix with the diagonals in columns \( [A] \)
		type(adZ_t),dimension(:),intent(in)::b
			!! Right-hand-side of the system \( \{b\} \)
		type(adZ_t),dimension(:),allocatable::x
			!! Problem solution \( \{x\} \)
		
		type(adZ_t),dimension(:,:),allocatable::W
		type(adZ_t)::r
		integer::N,k
	...
	subroutine decomposeLU(A,LU,p)
		...
		!! U -> j>=i
		type(adZ_t),dimension(:,:),intent(in)::A
		type(adZ_t),dimension(:,:),allocatable,intent(inout):: LU
	...
	function applyLU(LU,p,b) result(x)
		type(adZ_t),dimension(:,:),intent(in)::LU
		integer,dimension(:),intent(in)::p
		type(adZ_t),dimension(:),intent(in)::b
		type(adZ_t),dimension(:),allocatable::x
		
		type(adZ_t),dimension(:),allocatable::pb
		type(adZ_t),dimension(:),allocatable::r
		integer::N,i
		
		N  = size(b)
		allocate(r(N),x(N),pb(N))
		r  = adZ_t( (0.0_wp,0.0_wp) ,size(b(1)%grad()))
		x  = adZ_t( (0.0_wp,0.0_wp) ,size(b(1)%grad()))
		pb = b(p)
	...
	function solveLU_s(A,b) result(x)
		...
		type(adZ_t),dimension(:,:),allocatable::LU
